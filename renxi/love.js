(function(window){
    ...
    Tree.prototype = {
        ...
        flower: function(num) {
            var s = this, blooms = s.bloomsCache.splice(0, 2);
            for (var i = 0; i < blooms.length; i++) {
                s.addBloom(blooms[i]);
            }
            blooms = s.blooms;
        ...
        jump: function() {
            ...
            if ((blooms.length && blooms.length < 3) || !blooms.length) {
                var bloom = this.opt.bloom || {},
                    ...
                    figure = this.seed.heart.figure;
                var r = 240, x, y;
                for (var i = 0; i < random(8,30); i++) { //hoaroi
                    blooms.push(this.createBloom(width / 2 + width, height, r, figure, null, 1, null, 1, new Point(random(-100,600), 720), random(200,500)));
                }
            }
        }
    }
    ...
    Bloom = function(tree, point, figure, color, alpha, angle, scale, place, speed) {
        ...
        this.angle = angle || random(350,360);
